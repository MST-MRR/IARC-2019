#This is for Logging the "x" Velocity
import time
...
def init():
    f = open(SettingsLogging.xVelocity,'w') #opens a file (first variable) in write mode (w) and stores in f
...
def run():
    ...
    with open(SettingsLogging.xVelocity,'a') as g: #opens the same file as before but now in append mode (a)
