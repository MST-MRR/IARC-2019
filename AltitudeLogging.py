#This is for Logging the Altitude
import time
...
def init():
    f = open(SettingsLogging.altitude,'w') #opens a file (first variable) in write mode (w) and stores in f
...
def run():
    ...
    with open(SettingsLogging.altitude,'a') as g: #opens the same file as before but now in append mode (a)
