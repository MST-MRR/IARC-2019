...
def init():
    f = open(SettingsLogging.pitch,'w') #opens a file (first variable) in write mode (w) and stores in f
...
def run():
    ...
    with open(SettingsLogging.pitch,'a') as g: #opens the same file as before but now in append mode (a)
