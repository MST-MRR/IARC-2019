#This is for Logging the "z" Velocity
import time
...
def init():
    f = open(SettingsLogging.zVelocity,'w') #opens a file (first variable) in write mode (w) and stores in f
...
def run():
    ...
    with open(SettingsLogging.zVelocity,'a') as g: #opens the same file as before but now in append mode (a)
