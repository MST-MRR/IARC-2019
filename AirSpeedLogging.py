#This is for Logging the AirSpeed
import time
...
def init():
    f = open(SettingsLogging.airSpeed,'w') #opens a file (first variable) in write mode (w) and stores in f
...
def run():
    ...
    with open(SettingsLogging.airSpeed,'a') as g:#opens the same file as before but now in append mode (a)
